...
that are used for coping with CP-932 characters
unmapped to Unicode, C<SJIS_CALLBACK>.
(any reference will not allowed as C<STRING>.)

If C<SJIS_CALLBACK> is given, the second parameter
is used as C<STRING>; otherwise the first.
...
Currently, only coderefs are used as C<SJIS_CALLBACK>.
A string returned from C<SJIS_CALLBACK> is inserted
in place of unmapped characters.
...
(i.e. the leading byte string), the first argument is C<undef>
and the second argument is an unsigned integer representing the byte.
If not partial, the first argument is a defined string
...
If the first parameter is a reference,
that are used for coping with Unicode characters
unmapped to CP-932, C<UNICODE_CALLBACK>.
(any reference will not allowed as C<STRING>.)

If C<UNICODE_CALLBACK> is given, the second parameter
is used as C<STRING>; otherwise the first.
...
Currently, only coderefs are used as C<UNICODE_CALLBACK>.
...
This B<Unicode> is in the Perl's internal format (see L<perlunicode>).
...
are not implemented yet. They needs more investigation
...
Use C<utf8::decode>/C<utf8::encode> (see L<utf8.pm>) on Perl 5.7 or later
if necessary.
